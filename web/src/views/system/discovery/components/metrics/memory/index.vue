...
const ramData: any = inject('ramData');
const loading: any = inject('loading');

const getColor = computed(() => {
  if (ramData.percentage < 75) {
    return 'rgba(82,196,26,100%)';
  } else if (ramData.percentage < 89) {
  ...
  <div class="flex flex-col items-center justify-center">
    ...
    <Skeleton
      ...
      <div class="text-theme-content font-semibold text-3.5 mt-2">
        Memory <span class="text-theme-content font-normal mb-1">({{ ramData.used }}/{{ ramData.totalNuclear }}G)</span>
      </div>
    </Skeleton>
  </div>
</template>
