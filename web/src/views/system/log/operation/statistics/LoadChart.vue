...
import { BarData, DateType, PieData, PieSetting } from './types';
...
const getCountData = (group, data) => {
  ...
  for (let i = 0; i < group.length; i++) {
    ...
    // 所有来源只有枚举类型数据
    if (['resource', 'source', 'gender', 'type', 'status', 'real_name_status', 'target_type', 'receive_type', 'scope', 'send_type', 'api_type', 'method', 'send_status', 'push_status'].includes(cloum.key)) {
