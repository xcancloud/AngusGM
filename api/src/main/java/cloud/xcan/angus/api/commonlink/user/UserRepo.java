...
public interface UserRepo extends NameJoinRepository<User, Long>, BaseRepository<User, Long> {
   ...
   * ----------------------------------------------------
   * SELECT	* FROM	user0 WHERE tenant_id = 1 AND
   *  CASE
  ...
  @Query(value =
      "SELECT u.* FROM user0 u WHERE u.tenant_id =?1 AND u.enabled = 1 AND u.deleted = 0 "
          ...
          + "    UNION SELECT user_id FROM dept_user WHERE tenant_id = ?1 AND dept_id IN (?2) "
          + "    UNION SELECT user_id FROM group_user WHERE tenant_id = ?1 AND group_id IN (?2) "
          + "  ) uid "
          + ") ", countQuery =
      "SELECT count(*) FROM user0 u WHERE u.tenant_id =?1 AND u.enabled = 1 AND u.deleted = 0 "
          ...
          + "  ) uid "
          + ") ", nativeQuery = true)
  Page<User> findValidByOrgIdAndIdAndName(Long tenantId, Collection<?> orgIds,
      Collection<?> idsFilter,
  ...
  @Query(value =
      "SELECT u.* FROM user0 u WHERE u.tenant_id =?1 AND u.enabled = 1 AND u.deleted = 0"
          ...
          + "    UNION SELECT user_id FROM dept_user WHERE tenant_id = ?1 AND dept_id NOT IN (?2) AND case when CONCAT(?3,'') IS NOT NULL then id IN (?3) else 1=1 end"
          + "    UNION SELECT user_id FROM group_user WHERE tenant_id = ?1 AND group_id NOT IN (?2) AND case when CONCAT(?3,'') IS NOT NULL then id IN (?3) else 1=1 end"
          + "  ) uid "
          + ") ", countQuery =
      "SELECT count(*) FROM user0 u WHERE u.tenant_id =?1 AND u.enabled = 1 AND u.deleted = 0"
          + " AND case when CONCAT(?4,'') IS NOT NULL then id IN (?4) else 1=1 end AND case when CONCAT(?5,'') IS NOT NULL then fullname like CONCAT('%',?5,'%') else 1=1 end"
          ...
          + "  ) uid "
          + ") ", nativeQuery = true)
  Page<User> findValidByOrgIdNotAndIdAndName(Long tenantId, Collection<?> orgIds,
      Collection<?> containOrgIds, Collection<?> idsFilter, String nameFilter, Pageable pageable);
